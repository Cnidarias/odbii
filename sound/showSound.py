...
class getRGB:
    ...
    def shadesOfColor(self, color, data, colorIndex=-1, noSound=False):
        ...
        if colorIndex < 0:
            ...
            self.setSpecificColor(
                leftR, leftG, leftB, rightR, rightG, rightB, colorIndex
            )
    ...
    def shadesOfColorCircle(self, data, noSound=False):
        ...
        if self.timerChange >= self.timerAmount:
            ...
            if self.increasing == "R":
                self.changingColor0[0] += self.increasingRate
            elif self.increasing == "r":
                self.changingColor0[0] -= self.increasingRate
            elif self.increasing == "G":
                self.changingColor0[1] += self.increasingRate
            elif self.increasing == "g":
                self.changingColor0[1] -= self.increasingRate
            elif self.increasing == "B":
                self.changingColor0[2] += self.increasingRate
            elif self.increasing == "b":
                self.changingColor0[2] -= self.increasingRate
            ...
            if self.counter >= 255:
                self.counter = 0
                self.increasingIndex = (self.increasingIndex + 1) % len(
...
class CarColorChanger(qt.QWidget):
    ...
    def custUpdate(self):
        ...
        self.changeWidgetColor(
            self.wid0, self.color[0][0], self.color[0][1], self.color[0][2]
        )
        self.changeWidgetColor(
            self.wid1, self.color[1][0], self.color[1][1], self.color[1][2]
        )

        self.changeWidgetColor(
            self.wid2, self.color[2][0], self.color[2][1], self.color[2][2]
        )
        self.changeWidgetColor(
            self.wid3, self.color[3][0], self.color[3][1], self.color[3][2]
        )

        self.changeWidgetColor(
            self.wid4, self.color[4][0], self.color[4][1], self.color[4][2]
        )
